...
public class Bearstore_Register_TC03_VerifyRegisterFailedWithExistingUsername extends BaseTest{
    private static final Logger logger = LogManager.getLogger(Bearstore_Register_TC03_VerifyRegisterFailedWithExistingUsername.class);
    ...
    public void registerWithExistingUsername() {
        ...
        step_Verify_Error_Message_Displayed();
    }
    ...
    private void step_Verify_Error_Message_Displayed() {
        logger.info("Step 4 – Verify error message displayed");
        String actualError = registerStep.getErrorMessage();
        String expected = "The specified username already exists";
        
        logger.info("Expected error: {}", expected);
        logger.info("Actual error: {}", actualError);
        
        Assert.assertTrue(actualError.contains(expected), "Expected error message not displayed. Actual: " + actualError);
